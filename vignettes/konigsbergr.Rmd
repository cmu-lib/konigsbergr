...
`kongisbergr` is a playful experiment that allows you to find pathways across the bridges in your own city and town, relying on the data from [OpenStreetMap][osm].

## Getting Map Data

First, we need to get map data from OSM. `kongisbergr` takes data extracts produced by the [osmar] package, which provides several options for calling out to APIs or to loading files from disk.
